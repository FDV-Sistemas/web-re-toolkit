...
fn asset_key(url: &str) -> String {
    ...
    fn fetch_batch(&self, wanted: &[Want]) -> Vec<ClientResult<FetchResponse>> {
        ...
        let ttl = asset_cache_ttl();

        // Sem TTL configurado nada muda: busca tudo, como antes.
        let mut answers: Vec<ClientResult<FetchResponse>> = if let Some(ttl) = ttl {
            let mut pronto: Vec<Option<FetchResponse>> = vec![None; wanted.len()];
            {
                let mut cache = ASSET_CACHE.lock().unwrap_or_else(|error| error.into_inner());
                cache.retain(|_, (gravado, _)| gravado.elapsed() < ttl);
                for (indice, want) in wanted.iter().enumerate() {
                    if want.kind != Asset::Script {
                        continue;
                    }
                    if let Some((_, resposta)) = cache.get(&asset_key(&want.url)) {
                        pronto[indice] = Some(resposta.clone());
                    }
                }
            }

            let faltam: Vec<usize> = (0..wanted.len()).filter(|i| pronto[*i].is_none()).collect();
            let pedidos: Vec<FetchRequest> = faltam
                .iter()
                .map(|i| requests[*i].clone())
                .collect();
            let vieram = self.http.fetch_together(pedidos);

            {
                let mut cache = ASSET_CACHE.lock().unwrap_or_else(|error| error.into_inner());
                for (posicao, indice) in faltam.iter().enumerate() {
                    if wanted[*indice].kind != Asset::Script {
                        continue;
                    }
                    if let Ok(resposta) = &vieram[posicao] {
                        if resposta.status == 200 {
                            cache.insert(
                                asset_key(&wanted[*indice].url),
                                (Instant::now(), resposta.clone()),
                            );
                        }
                    }
                }
            ...
            let mut vieram = vieram.into_iter();
            pronto
                .into_iter()
                .map(|guardado| match guardado {
                    Some(resposta) => Ok(resposta),
                    None => vieram.next().expect("uma resposta por item faltante"),
                })
                .collect()
        } else {
            self.http.fetch_together(requests)
        };
        let answers = std::mem::take(&mut answers);
